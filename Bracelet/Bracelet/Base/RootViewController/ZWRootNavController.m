...
#import "ZWTranstionProtocal.h"
#import "ZWTRansition.h"
#import "ZWRootController.h"

@interface ZWRootNavController ()<UINavigationControllerDelegate,UIGestureRecognizerDelegate>
...
@implementation ZWRootNavController
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    if (self.viewControllers.count > 0) {
        if ([viewController conformsToProtocol:@protocol(ZWTransitionProtocol)] && [self isNeedTransition:viewController]) {
...
-(void)navigationController:(UINavigationController *)navigationController willShowViewController:(UIViewController *)viewController animated:(BOOL)animated{
    ...
    if ([viewController isKindOfClass:[ZWRootController class]]) {
        ZWRootController * vc = (ZWRootController *)viewController;
...
-(id<UIViewControllerAnimatedTransitioning>)navigationController:(UINavigationController *)navigationController animationControllerForOperation:(UINavigationControllerOperation)operation fromViewController:(UIViewController *)fromVC toViewController:(UIViewController *)toVC
{
    ...
    if ([fromVC conformsToProtocol:@protocol(ZWTransitionProtocol)] && [toVC conformsToProtocol:@protocol(ZWTransitionProtocol)]) {
        
        BOOL pinterestNedd = [self isNeedTransition:fromVC:toVC];
        ZWTRansition *transion = [ZWTRansition new];
        ...
        return transion;
    }else if([toVC conformsToProtocol:@protocol(ZWTransitionProtocol)]){
...
-(BOOL)isNeedTransition:(UIViewController<ZWTransitionProtocol> *)fromVC :(UIViewController<ZWTransitionProtocol> *)toVC
{
    BOOL a = NO;
    BOOL b = NO;
...
//判断fromVC和toVC是否需要实现pinterest效果
-(BOOL)isNeedTransition:(UIViewController<ZWTransitionProtocol> *)toVC
{
    BOOL b = NO;
    if ([toVC respondsToSelector:@selector(isNeedTransition)] && [toVC isNeedTransition]) {
        b = YES;
